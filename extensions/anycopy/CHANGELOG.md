...
- **Range selection.** `Shift+V` starts an inclusive range that extends or shrinks with normal tree movement. The default `toggle` mode reverses every node against its initial state, while the optional `select` mode provides classic additive selection. Copying finishes range mode while preserving its result
